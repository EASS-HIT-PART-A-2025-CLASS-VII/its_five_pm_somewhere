...
def test_generate_drink_success():
    ...
    try:
        ...
        # Mock dependencies
        with patch.object(mixology_agent, 'run_sync', return_value=mock_ai_result), \
             patch('backend.app.main.get_pexels_images', return_value=[mock_image_url]):
            
            # Call endpoint
            response = client.post(
                "/drinks/generate",
                json=["apple juice", "lime", "mint"]
            )
            ...
            assert any(ing.name.lower() == "apple juice" for ing in returned_drink.ingredients)
